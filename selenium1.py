...
class imgVerify:
    ...
    def getCoordByChaoying(self,driver):
        ...
        if self.img == "D:/spider/captche/orderClick.png":
            ...
            dict =self.chaojiying.PostPic(im, 9102)  # 1902 验证码类型  官方网站>>价格体系 3.4+版 print 后要加()
            print(dict)
            if dict['err_str'] =='OK':
                return dict
    ...
    def verify(self,driver):
        ...
        if self.img == "D:/spider/captche/orderClick.png":
            ...
            #判断是否验证成功
            if not self.isCoorRight(driver,int(dict['pic_id'])):
                self.verify(driver)
            else:
                print('登录成功')
            #返回主页面
    ...
    def isCoorRight(self,driver,im_id):
        ...
        try:
            ...
            print('验证失败，重新验证,并且提交错误图片ID返回积分')
...
if 'acf_username' not in str(driver.get_cookies()):
    # 输入用户名密码
    imgVerifyobj.infoInput(driver, '斗鱼手机号', '斗鱼密码')
    # 验证登录
